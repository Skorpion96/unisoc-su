...
Now, let's talk about about this exploit, the context is heavily guarded by selinux, we have root but every protection is still up. This root is huge because we didn't disable anything to get it like other similar exploits. Unfortunately this context doesn't have enough power to disable selinux and also execution seems to work only on system PATH. About the service itself, seems on Android 9 (so before [CVE-2022-47339](https://nvd.nist.gov/vuln/detail/cve-2022-47339) patch) it doesn't have groups on it's service rc and so them do default to root, later instead groups got added (and root as gid/groups removed) so it's obvious the service got more restricted, but with selinux up it's this one which rules anyway. About how the service behaves, which means how it acts: on new models it seems to run until something uses it (or is connected to it), if there is no client connected the service will turn off and will be needed the setprop property to turn it back on again, the service does this almost immediately, this is why on this method we run the setprop and fastly connect, on Android 9 user builds the service seems to wait for a command, after execution it will turn off, of course if you just connect to it with socat or with the [cli-pie](https://github.com/TomKing062/cmd_services_client/releases) (or run the bridge) the service will remain up as it will be occupied by this connection, if no command is provided the service will remain waiting indefinitely. Instead on eng at least for what i tried seems once cmd_services is on won't turn off, at least on Android 9 (i don't know it's behaviour on newer Android versions). 
